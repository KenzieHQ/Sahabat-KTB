-- Simple Admin Setup (After fixing login issues)
...
DROP POLICY IF EXISTS "Allow admins to delete profiles" ON user_profiles;
...
-- Step 3: Add kenzie.siregar@sma-ktb.sch.id as admin
DO $$
DECLARE
    admin_user_id UUID;
BEGIN
...
DROP POLICY IF EXISTS "Allow users and admins to delete posts" ON posts;
...
DROP POLICY IF EXISTS "Allow users and admins to delete replies" ON replies;
...
-- Step 6: Populate existing users into user_profiles
